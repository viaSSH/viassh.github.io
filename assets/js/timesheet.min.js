(function() {
    ...
    for (var n = 0, m = this.data.length; n < m; n++) {
      ...
      if(cur.lr === 1) {
        var line = [
          ...
          '<span style="margin-left:5px; margin-right: ' + bubble.getReverseOffset() + 'px; width: ' + bubble.getWidth() + 'px; float:right; " class="bubble bubble-' + (cur.type || 'default') + '" data-duration="' + (cur.end ? Math.round((cur.end-cur.start)/1000/60/60/24/39) : '') + '"></span>',
          '<span class="date" style="padding-left:5px; float: right; ">' + bubble.getDateLabel() + '</span> ',
          '<span class="label" style="float: right; ">' + cur.label + '</span>',
        ].join('');
      }
      else{
        var line = [
  ...
  Timesheet.prototype.parse = function(data) {
    for (var n = 0, m = data.length; n<m; n++) {
      ...
      // custom by viassh
      var end = this.parseDate(data[n][1]);
      var lbl = data[n][2];
      var cat = data[n][3];
      var lr = data[n].length === 5 ? data[n][4] : 0;
      ...
      this.data.push({start: beg, end: end, label: lbl, type: cat, lr: lr});
    }
  };
  ...
  // Calc reverse offset - by viassh
  Bubble.prototype.getReverseOffset = function() {
    // console.log(this.max);
    // let
    return (this.max-this.min+1)*this.widthMonth - ( (this.widthMonth/12) * (12 * (this.start.getFullYear() - this.min) + this.start.getMonth()) + (this.widthMonth/12) * this.getMonths());
  };
